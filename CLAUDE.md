...
|---|---|---|
| `validate_quiz.py` | 1 (`[A VERIFIER]`), 2 (ligatures/PUA), 9 (`.wrap`/`.hwrap`), 3 (titres de section), cohérence `data-correct`↔options, **fusion de questions** (en-tête `Question N: (Type:` fondu dans un bloc, lettre d'option `data-l` en double, lettre répétée dans `data-correct`), 6 (**image annoncée mais absente** : énoncé/`dpctx` mentionnant « ci-dessous », « cf image », « … est la suivante : » sans `<div class="extra">`) | Contrôles **structurels/techniques**, 0 token, exit 1 si bloquant (image manquante = avertissement). |
...
6. **Images** : le script associe chaque image détectée à la première question non pourvue de la même page PDF (heuristique best-effort). Vérifier visuellement que chaque image est sur la bonne question et bien positionnée (après `.stem`, avant `.opts`). `pdf_to_quiz.py` **signale désormais** toute question dont l'énoncé/le contexte annonce une image (« ci-dessous », « cf image », « … est la suivante : ») mais à laquelle aucune image n'a été associée — cas typique d'une image **non extractible** (vectorielle/non bitmap, ou filtrée par la taille < 100 px) qui, avant, sortait silencieusement absente. `validate_quiz.py` refait ce contrôle (`IMAGE_MISSING`). Si une image manque, la récupérer à la main depuis le PDF source (PyMuPDF `page.get_pixmap()` sur la zone si `get_images()` ne la voit pas) et l'embarquer en base64.
