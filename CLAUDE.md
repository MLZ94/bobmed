...
- Injecte automatiquement les **quatre scripts globaux** juste avant `</body>`, dans l'ordre standard des annales : `breadcrumb.js`, `dynamic-header.js`, `timer.js`, `progress.js` (avec le bon préfixe relatif selon la profondeur du dossier de destination). Toute annale générée charge donc le minuteur d'examen sans intervention manuelle.
