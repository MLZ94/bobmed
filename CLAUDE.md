...
- **Mode sombre — mini-script de bascule INLINE, obligatoire sur toute page** : le choix clair/sombre se fait sur la page d'accueil (`localStorage.theme`) ; chaque page applique ensuite la classe `html.dark` via ce script placé **dans le `<head>`**, juste après `</style>` (il doit s'exécuter avant le rendu, sinon flash clair — c'est la seule exception à la règle « jamais de copier-coller », car un fichier externe arriverait trop tard) :
  ...
  ```
  Sans lui, la page ignore le mode sombre (bug historique : le gabarit de `pdf_to_quiz.py` l'omettait → 12 annales + 40 quiz d'entraînement T3/T4 restés clairs, corrigés). `pdf_to_quiz.py` l'injecte désormais (constante `DARK_MODE_BOOTSTRAP`) et `validate_quiz.py` **bloque** toute page qui ne l'a pas (`DARK_MODE_MISSING`). Toute page créée à la main doit l'inclure.
...
4. `<div class="extra">` — image (optionnel). **Si le PDF intercale du texte entre les images** (texte → image → texte → image), reproduire cet ordre avec plusieurs `.stem` : `.stem` → `.extra` → `.stem` → `.extra` (cf. « Images »)
...
- Traiter comme QRU (`data-type="QRU"`, badge `TCS`, choix unique)
- Options fixes : A = improbable · B = peu probable · C = ni plus ou moins probable · D = probable · E = certain (ou l'échelle « complètement contre-indiqué … fortement indiqué »)
- **Source de vérité = le fichier réponse de l'annale** (cf. « Fichier réponse de l'annale ») : chaque option marquée **« Valide »** est une réponse retenue par au moins un expert du panel, donc une réponse **juste qui rapporte des points**.
- **Barème pondéré du panel (Uness/R2C)** : la réponse majoritaire vaut 1/1, les autres réponses validées une fraction (proportion d'experts). Ce n'est **pas** 1/n : relevé sur les annales du dépôt, 0,09 · 0,29 · 0,33 · 0,71 · 0,8… La note de l'étudiant source sur la question (« `Question 1: (Type: QTCS) 0.33/1` ») est le **poids de l'option qu'il a cochée** : une option cochée « Valide » mais notée < 1 est juste mais non majoritaire.
...
function grade(q) {
  ...
  // TCS pondérée (plusieurs réponses validées par le jury, cf. « TCS ») : points =
  // poids data-w de l'option choisie, 0 si non validée ; les autres réponses
  // validées sont signalées « validée » (et non « manqué »).
  const isW = !!q.querySelector('.opt[data-w]');
  if (isW) {
    const _o = [...q.querySelectorAll('.opt')].find(o => sel.has(o.dataset.l));
    pts = _o && _o.dataset.w !== undefined ? parseFloat(_o.dataset.w) : 0;
    q.querySelectorAll('.opt.missed .mark').forEach(m => { m.textContent = 'validée'; });
  }
...
- **Exception — texte intercalé entre les images dans le PDF** : respecter l'ordre du PDF en découpant l'énoncé en plusieurs `.stem`, ex. SQI1-Q1 UE8.2 juillet 2024 : `.stem` (« … Le premier ECG … est celui-ci : ») → `.extra` (ECG 1) → `.stem` (« Le suivant est réalisé après ralentissement de la fréquence cardiaque : ») → `.extra` (ECG 2) → `.opts`. Idem pour le cas fréquent « description … suivante : » [image] « Quelle est votre hypothèse ? ». Ne jamais recoller dans l'énoncé une phrase qui, dans le PDF, suit une image. `pdf_to_quiz.py` le fait seul (fonction `stem_with_images`) ; il ne coupe qu'à une **frontière de phrase** — une image repoussée en haut de page au milieu d'une phrase (« … à l'aide de » [image] « cette grille ? ») garde le placement standard, après l'énoncé.
...
### Fichier réponse de l'annale (source de vérité — à consulter systématiquement)

Les PDF sources vivent dans le dépôt `bobmed-annales-drive` (`T1/Cardiologie/2023-2024/…`), où une même session a souvent **deux** fichiers côte à côte :

| Fichier | Contenu | Usage |
|---|---|---|
| **Sujet** — `sujet N ….pdf`, `… - Sujet.pdf`, `… vierge.pdf` | copie de l'étudiant : cases ☐/☑ ◎/◉ **sans** libellé de validité | ❌ jamais comme source (aucune bonne réponse) |
| **Réponse** — `réponse N ….pdf`, `correction N.pdf`, `… - Corrigé.pdf`, `…Annales-corrigé….pdf` | chaque option précédée de son libellé **Valide / Faux / Indispensable / Inacceptable / Neutraliser** (en bleu), note obtenue par question (`Question 1: (Type: QTCS) 0.33/1`), notes du jury | ✅ **seule source de vérité** |

Pour identifier la session, lire la ligne `Epreuve:` en page 1 (ex. `DFA1-UE8.2-JUILLET2024-RATTRAPAGE`) : le sujet et la réponse d'une même session portent le même code, le numéro du nom de fichier (`sujet 2` ↔ `réponse 2`) n'est qu'un indice.

**Règle** : toute question sur une bonne réponse, un item indispensable/inacceptable, une TCS ou une note se tranche **en ouvrant le fichier réponse de l'annale concernée** — jamais le sujet, jamais une déduction à partir du texte seul. En particulier :
...
| 2 (fidélité texte), 7 (`data-correct` vs options valides du PDF, TCS à réponses multiples), items indispensable/inacceptable option par option, types | `quiz_agent.py` (+ garde-fou intégré à `pdf_to_quiz.py`, exit 3) |
| Mode sombre (script de bascule), moteur indispensable/inacceptable et TCS pondérée | `validate_quiz.py` |
...
1. **Marqueurs `[A VERIFIER]`** : chercher toute occurrence dans le fichier (option sans lettre détectée, QROC sans réponse attendue, QRPL sans nombre de réponses détecté, **incohérence sur les items indispensable/inacceptable** — commentaire en tête de `<body>`, exit 3 du script) et les résoudre à la main à partir du fichier réponse.
